...
<html lang="en">
  <head>
    ...
    <title>
      {{ $title ?? 'Online Service Portal' }} | TailAdmin - Tailwind CSS Admin Dashboard Template
    </title>
...
<style>
  ...
  <body
    x-data="{
        page: '{{ request()->routeIs('portal.profile') ? 'profile' : 'dashboard' }}',
        loaded: true,
  ...
  >
    @php
        $isAuthenticated = session()->has('api_token') && session()->has('sso_payload');
        $isAuthRoute = request()->routeIs('login') || request()->routeIs('login.*');
    @endphp
    ...
    @if ($isAuthenticated && $isAuthRoute)
        ...
        <noscript>
            <meta http-equiv="refresh" content="0;url={{ route('portal.dashboard') }}">
        </noscript>
    @elseif (!$isAuthenticated && !$isAuthRoute)
